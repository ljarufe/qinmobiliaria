...
class Watermarker(object):
    ...
    def __call__(self, url, proyecto_id, position=None, opacity=0.5, tile=False,
        ...
        * ``quality``: the quality of the resulting watermarked image.  Default
        ...
        if proyecto.logo_watermark:
            watermark = proyecto.logo_watermark.thumbnail.relative_url
        else:
            watermark = Inmobiliaria.objects.latest("id").logo_watermark.thumbnail.relative_url
        watermark = "%s%s" % (settings.MEDIA_ROOT, watermark)
