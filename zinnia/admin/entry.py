...
from common.models import Foto, Video
from common.utils import json_response
from proyectos.forms import AdminFotoForm, AdminDeleteFotoForm, AdminVideoForm, AdminDeleteVideoForm
...
class EntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((_('Content'), {'fields': ('title', 'content', 'fuente',
                                            'image', 'proyectos', 'status')}),
                 ...
                 (_('Publication'), {'fields': ('categories', 'tags',
                                                'sites', 'slug')}))
    list_filter = ('proyectos', 'status', 'featured',
    ...
    filter_horizontal = ('proyectos', 'authors', 'related')
    ...
    def queryset(self, request):
        ...
        # """
        # solo los super usuarios tienen acceso a todas las entradas de todos 
        # los proyectos, los demas usuarios solo tendran acceso a las entradas
        # de los proyectos donde estan asociados
        # """
        qs = super(self.__class__, self).queryset(request)
        if request.user.is_superuser:
            return qs;
        return qs.filter(proyectos__usuarios__id=request.user.id).distinct()
    ...
    def add_fotos(self, request, entry_id=''):
        """
        ...
        if entry_id:
            ...
            fotosList = entry.fotos.iterator()
            fotosLength = entry.fotos.count()
        add_success = False
        del_success = False
        form = AdminFotoForm()
    ...
    def add_videos(self, request, entry_id=''):
        """
        ...
        if entry_id:
            entry= get_object_or_404(Entry, id=entry_id)
            videosList = entry.videos.iterator()
            videosLength = entry.videos.count()
        add_success = False
        del_success = False
        form = AdminVideoForm()
