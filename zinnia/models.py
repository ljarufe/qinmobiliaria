...
from common.models import Fuente
...
class EntryAbstractClass(models.Model):
    ...
    fuente = models.ForeignKey(Fuente, verbose_name="Fuente", null=True,
                               blank=True)
    excerpt = models.TextField(_('excerpt'), blank=True,
                                help_text=_('optional element'))
    image = ImageWithThumbnailsField(
        _(u'imágen principal'),
        upload_to="entries/fotos",
        generate_on_save=True,
        thumbnail={'size': (100, 100), 'options': ['crop', 'upscale']},
        extra_thumbnails={
            'small': {'size': (25, 25), 'options': ['crop', 'upscale']},
            'small_preview': {'size': (120, 110), 'options': ['crop', 'upscale']},
            'big': {'size': (426, 295), 'options': ['crop', 'upscale']},
        }
    )
